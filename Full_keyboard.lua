...
-- [1] IMPROVED FORCE MOBILE CONTROLS
task.spawn(function()
        ...
        pcall(function()
            ...
            if touchGui then
                ...
                if touchFrame then 
                    ...
                    -- التأكد من أن الجويستيك قابل للمس دائماً
                    touchFrame.Active = false
                end
            end
        end)
    end
...
ScreenGui.Name = "FinalUltimateKeyboard_V7"
...
Holder.Active = false -- IMPORTANT: Allows touch to pass to joystick
...
local pickBtn = createCtrl("Choose Key", Color3.fromRGB(160, 140, 0), 5, 85, function() pickingMode = not pickingMode deleteMode = false end)
local delBtn = createCtrl("Del Mode: OFF", Color3.fromRGB(100, 0, 0), 95, 95, function() deleteMode = not deleteMode pickingMode = false end)
local lockGuiBtn = createCtrl("Lock GUI: OFF", Color3.fromRGB(0, 80, 150), 195, 95, function() guiLocked = not guiLocked Holder.Draggable = not guiLocked end)
local lockKeysBtn = createCtrl("Lock Keys: OFF", Color3.fromRGB(0, 120, 80), 295, 95, function() 
    keysLocked = not keysLocked 
    for _, v in pairs(ScreenGui:GetChildren()) do
        if v.Name == "CustomKey" then v.Draggable = not keysLocked end
    end
end)
createCtrl("Hide GUI", Color3.fromRGB(170, 0, 0), 395, 80, function() Holder.Visible = false end)

-- [4] KEYBOARD LOGIC
local function spawnExternalKey(name)
    local k = Instance.new("TextButton")
    k.Name = "CustomKey"
    k.Size = UDim2.new(0, 55, 0, 55)
    ...
    k.Active = true
    -- Modal = false ضروري جداً هنا لكي لا يعطل الجويستيك
    k.Modal = false 
    k.Parent = ScreenGui
